...
public class iFS {
    ...
    private AtomicReference<List<String>> sets = new AtomicReference<>(new ArrayList<>());
...
//        }
        ...
        while (true) {
            List<String> got = sets.get();
            if (got.size() == 0) continue;
            String something = got.get(0);
            got.remove(0);
...
//            System.out.println("something = " + something);
            ...
            executorService.execute(() -> {
                try {
                    downloadSet(shuffle ? "/feeds/shuffle" : something);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            });
    ...
    private void startTimeThread() {
        new Thread(() -> {
            while (true) {
                ...
                if (memes.get() > 0) System.out.println("Collected " + memes.get() + " @" + (memes.get() / timePassed) + "mps (" + ((memes.get() - duplicates.get()) / timePassed) + "umps) \tover " + timePassed + " seconds \tDuplicate percentage: " + (Math.round(((double) duplicates.get() / (double) memes.get() * 100D) * 100D) / 100D) + "%");
            }
        }).start();
    }
